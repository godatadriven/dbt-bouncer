...
with
    default_data as (
        ...
        select
            cast(
                {{ dbt.string_literal("2022-01-01") }} as {{ dbt.type_timestamp() }}
            ) as date_expected,
            cast(
                {{
                    dbt.string_literal(
                        dbt_utils.get_single_value(false_statement, "2022-01-01")
                    )
            ...
            cast(
                {{ dbt.string_literal("fallback") }} as {{ dbt.type_string() }}
            ) as string_expected,
            cast(
                {{
                    dbt.string_literal(
                        dbt_utils.get_single_value(false_statement, "fallback")
                    )
                }} as {{ dbt.type_string() }}
            ) as string_actual
