{% macro print_dbt_project_evaluator_issues(format="table", quote="") %}
    ...
    -#}
        ...
        {% for result in results | selectattr("failures") | selectattr(
            "failures", ">", 0
        ) %}
            ...
            {% if is_test and (
                ...
                {% set model_database = (
                    quote ~ model_details.database ~ quote
                    if model_details.database
                    else None
                ) %}
                {% set db_schema = (
        ...
        select * from {{db_schema}}.{{name_model_checked}}
                ...
                {% if format == "table" %}
                    {{
                        print(
                            query_results.print_table(
                                max_column_width=80, max_rows=1000
                            )
                            or ""
                        )
                    }}
                {% elif format == "csv" %} {{ print(query_results.print_csv() or "") }}
                {% else %}
                    {%- do exceptions.raise_compiler_error(
                        "format can only be 'table' or 'csv'"
                    ) -%}
                {% endif %}
