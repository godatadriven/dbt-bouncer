...
{%- macro default__get_node_values() -%}
    ...
    {%- if execute -%}
        ...
        {%- for node in nodes_list -%}
            ...
            {%- set hard_coded_references = (
                dbt_project_evaluator.find_all_hard_coded_references(node)
            ) -%}
            {%- set number_lines = dbt_project_evaluator.calculate_number_lines(
                node
            ) -%}
            {%- set sql_complexity = dbt_project_evaluator.calculate_sql_complexity(
                node
            ) -%}
            {%- set contract = node.contract.enforced if node.contract else false -%}
            {%- set exclude_node = dbt_project_evaluator.set_is_excluded(
            ...
            {%- set values_line = [
                ...
                wrap_string_with_quotes(
                    node.original_file_path | replace("\\", "\\\\")
                ),
                ...
                "cast("
                ~ dbt_project_evaluator.is_not_empty_string(node.description)
                | trim ~ " as boolean)",
                (
                    "''"
                    if not node.column_name
                    else wrap_string_with_quotes(
                        dbt.escape_single_quotes(node.column_name)
                    )
                ),
                wrap_string_with_quotes(node.meta | tojson),
                wrap_string_with_quotes(
                    dbt.escape_single_quotes(hard_coded_references)
                ),
                number_lines,
                sql_complexity,
                wrap_string_with_quotes(
