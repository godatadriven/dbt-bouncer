{%- macro get_relationship_values(node_type) -%}
    {{
        return(
            adapter.dispatch("get_relationship_values", "dbt_project_evaluator")(
                node_type
            )
        )
    }}
        ...
        {%- else -%}
            {{
                exceptions.raise_compiler_error(
                    "node_type needs to be either nodes, exposures or metrics, got "
                    ~ node_type
                )
            }}
        {% endif -%}
            ...
            {%- else -%}
                ...
                {%- for parent in node.get("depends_on", {}).get("nodes", []) -%}
                    ...
                    {%- set values_line = [
                        ...
                        (
                            "" ~ loop.last ~ ""
                            if node.unique_id.split(".")[0] == "test"
                            else "FALSE"
                        ),
                    ] %}
