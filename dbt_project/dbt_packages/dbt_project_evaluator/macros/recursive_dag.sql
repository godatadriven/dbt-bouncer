...
{% macro default__recursive_dag() %}
    ...
    with recursive
        ...
        -- recursive CTE
        -- one record for every resource and each of its downstream children
        -- (including itself)
        all_relationships(
            parent_id,
            parent,
            ...
            -- where direct_parent_id is null {# optional lever to change filtering of
            -- anchor clause to only include root resources #}
            union all
            ...
            select
                ...
                {{
                    dbt.array_append(
                        "all_relationships.path", "direct_relationships.resource_name"
                    )
                }} as path,
                case
                    when
                        ...
                        and coalesce(
                            all_relationships.is_dependent_on_chain_of_views, true
                        )
                    then true
                    else false
...
{% macro bigquery__recursive_dag() %}
    ...
    -- as of Feb 2022 BigQuery doesn't support with recursive in the same way as other
    -- DWs
    ...
    with
        ...
        -- must do distinct prior to creating array because BigQuery doesn't support
        -- distinct on array type
        get_distinct as (
            select distinct
                resource_id as parent_id,
                resource_id as child_id,
        ...
        {% for i in range(1, max_depth) %}
            ...
            cte_{{ i }} as (
                select
                    ...
                    {{
                        dbt.array_append(
                            prev_cte_path, "direct_relationships.resource_name"
                        )
                    }} as path,
                    case
                        when
                            ...
                            and coalesce(
                                cte_{{ i - 1 }}.is_dependent_on_chain_of_views, true
                            )
                        then true
                        else false
                    end as is_dependent_on_chain_of_views
        ...
        resource_info as (select * from {{ ref("int_all_graph_resources") }}),
...
{% macro spark__recursive_dag() %}
    -- as of June 2022 databricks SQL doesn't support "with recursive" in the same way
    -- as other DWs
