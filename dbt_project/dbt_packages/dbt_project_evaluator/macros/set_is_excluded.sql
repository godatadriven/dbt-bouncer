{% macro set_is_excluded(resource, resource_type) %}
    {{
        return(
            adapter.dispatch("set_is_excluded", "dbt_project_evaluator")(
...
{% macro default__set_is_excluded(resource, resource_type) %}
    ...
    {% else %}
        {{
            exceptions.raise_compiler_error(
                "`set_is_excluded()` macro does not support resource type: "
                ~ resource_type
            )
        }}
    {% endif %}
    ...
    {%- for exclude_paths_pattern in var("exclude_paths_from_project", []) -%}
        {%- set matched_path = re.search(
            exclude_paths_pattern, resource_path, re.IGNORECASE
        ) -%}
    ...
    {%- if (resource.package_name != project_name) and (
        resource.package_name in var("exclude_packages", [])
        or "all" in var("exclude_packages", [])
    ) -%}
        {% set ns.exclude = true %}
    {%- endif -%}
