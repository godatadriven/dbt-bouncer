{% macro calculate_sql_complexity(node) %}
    {{
        return(
            adapter.dispatch("calculate_sql_complexity", "dbt_project_evaluator")(
                node
            )
        )
    }}
    ...
    {% if node.resource_type == "model" and node.language == "sql" %}
        ...
        {%- for token, token_cost in var("token_costs").items() -%}
            ...
            {%- set all_regex_matches = re.findall(
                ...
                model_raw_sql_no_comments,
                re.IGNORECASE,
            ) -%}
            {%- set ns.complexity = ns.complexity + token_cost * (
                all_regex_matches | length
            ) -%}
