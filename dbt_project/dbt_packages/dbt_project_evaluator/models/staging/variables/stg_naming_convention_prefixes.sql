...
with
    vars_prefix_table as ({{ dbt_project_evaluator.loop_vars(vars_prefix) }}),
    ...
    parsed as (
        ...
        select
            ...
            {{
                dbt.replace(
                    "var_name", wrap_string_with_quotes(suffix_model_type), "''"
                )
            }} as model_type,
    ...
    final as (
        ...
        select
            {{ dbt_utils.generate_surrogate_key(["model_type", "prefix_value"]) }}
            as unique_id,
            *
        from parsed
