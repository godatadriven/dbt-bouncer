with
    all_dag_relationships as (
        ...
        where not parent_is_excluded and not child_is_excluded
    ),
    ...
    -- all parents with more direct children than the threshold for fanout (determined
    -- by variable models_fanout_threshold, default 3)
    -- Note: only counts "leaf children" - direct chilren that are models AND are
    -- child-less (are at the right-most-point in the DAG)
    model_fanout as (
        select
        ...
        from all_dag_relationships
        inner join
            models_without_children
            on all_dag_relationships.child = models_without_children.parent
        where
            all_dag_relationships.distance = 1
            and all_dag_relationships.child_resource_type = 'model'
        group by 1, 2, 3
        -- we order the CTE so that listagg returns values correctly sorted for some
        -- warehouses
        order by 1, 2, 3
    ),
