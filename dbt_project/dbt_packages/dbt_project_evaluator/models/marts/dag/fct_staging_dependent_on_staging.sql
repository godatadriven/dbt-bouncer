...
with
    direct_model_relationships as (
        ...
        where
            parent_resource_type in ('model', 'snapshot')
    ...
    bending_connections as (
        select parent, parent_model_type, child, child_model_type
        from direct_model_relationships
        where parent_model_type = 'staging' and child_model_type = 'staging'
    )

select *
from bending_connections {{ filter_exceptions() }}
