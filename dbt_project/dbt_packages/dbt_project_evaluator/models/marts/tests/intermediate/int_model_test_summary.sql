with
    ...
    relationships as (select * from {{ ref("int_direct_relationships") }}),
    ...
    count_column_tests as (
        ...
        select
            ...
            sum(
                case when all_graph_resources.is_test_unique then 1 else 0 end
                    ...
                    distinct case
                        when
                            {%- for test in test_set %}
                                all_graph_resources.is_{{ test.split(".")[1] }}
                                {%- if not loop.last %} or {% endif %}
                            {%- endfor %}
                        then relationships.resource_id
                        else null
                    end
        ...
        left join
            relationships on all_graph_resources.resource_id = relationships.resource_id
        where
    ...
    count_column_constraints as (
        ...
        select
            ...
            name as column_name,
            case
                when has_not_null_constraint then 1 else 0
    ...
    combine_column_counts as (
        ...
        select
            count_column_tests.*,
            count_column_tests.test_unique_count
            + count_column_constraints.constraint_not_null_count
            as primary_key_mixed_method_count,
            count_column_constraints.constraints_count
        from count_column_tests
        left join
            count_column_constraints
            on count_column_tests.direct_parent_id
            = count_column_constraints.direct_parent_id
                    ...
                    when
                        (
                            {%- for test_set in var("primary_key_test_macros") %}
                                ...
                                primary_key_method_{{ loop.index }}_count
                                >= {{ compare_value }}
                                or
                            {%- endfor %}
    ...
    final as (
        select
            ...
            coalesce(
                agg_test_relationships.is_primary_key_tested, false
            ) as is_primary_key_tested,
            coalesce(
                agg_test_relationships.number_of_tests_on_model, 0
            ) as number_of_tests_on_model,
            coalesce(
                agg_test_relationships.number_of_constraints_on_model, 0
            ) as number_of_constraints_on_model
        from all_graph_resources
        left join
