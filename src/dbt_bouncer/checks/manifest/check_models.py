...
class CheckModelDescriptionPopulated(BaseCheck):
    name: Literal["check_model_description_populated"] = Field(default=None)

    @pytest.mark.iterate_over_models
    def check_model_description_populated(self, request, check_config=None, model=None):
        """
...
class CheckModelNames(BaseCheck):
    ...
    name: Literal["check_model_names"] = Field(default=None)
    model_name_pattern: str = Field(default=None, description="Regexp the model name must match.")

    @pytest.mark.iterate_over_models
    def check_model_names(self, request, check_config=None, model=None):
        """
