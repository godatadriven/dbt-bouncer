...
@check(
    ...
    iterate_over="snapshot",
    params={
        "criteria": (str, "all"),
        "tags": list[str],
    },
)
def check_snapshot_has_tags(snapshot, ctx, *, criteria: str, tags: list[str]):
...
@check(
    ...
    iterate_over="snapshot",
    params={"snapshot_name_pattern": str},
)
def check_snapshot_names(snapshot, ctx, *, snapshot_name_pattern: str):
