...
class CheckSourceHasMetaKeys(BaseCheck):
    keys: Optional[Union[Dict[str, Any], List[Any]]] = Field(default=None)
    name: Literal["check_source_has_meta_keys"] = Field(default=None)

    @pytest.mark.iterate_over_sources
    def check_source_has_meta_keys(self, request, check_config=None, source=None) -> None:
        """
