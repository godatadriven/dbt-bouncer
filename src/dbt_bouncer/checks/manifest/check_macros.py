...
def check_macro_arguments_description_populated(
    macro, ctx, *, min_description_length: int | None
):
...
@check(
    ...
    iterate_over="macro",
    params={"max_number_of_lines": (int, 50)},
)
def check_macro_max_number_of_lines(macro, ctx, *, max_number_of_lines: int):
...
def check_macro_name_matches_file_name(macro, ctx):
...
def check_macro_property_file_location(macro, ctx):
