...
class CheckProjectName(BaseCheck):
    name: Literal["check_project_name"] = Field(default=None)
    project_name_pattern: str = Field(
        default=None, description="Regexp the project name must match."
    )

    def check_project_name(self, manifest_obj, request, check_config=None):
        """
