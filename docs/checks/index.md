`dbt-bouncer` runs checks against artifacts from dbt. Every check also has a unique [rule code](./rule_codes.md) (e.g. `MO019`) that can be used in place of its name. These checks fall into three categories:
