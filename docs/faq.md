...
- To compare against a previous run without a stored file, use `--state <dir>` in place of `--baseline`. It points at a directory of dbt artifacts from a previous run, the same way dbt's own `--state` flag does. See the [CLI reference](cli.md).
