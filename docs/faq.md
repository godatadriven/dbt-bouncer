...
Yes. dbt-core 2.0 (the Rust-based [Fusion](https://docs.getdbt.com/docs/fusion/about-fusion)
engine) emits the same `manifest.json`, `catalog.json` and `run_results.json` artifacts —
at manifest schema v12 — as dbt-core 1.x. Because `dbt-bouncer` consumes those artifacts
directly, every check that works against dbt-core also works against dbt 2.0. Our CI builds the
