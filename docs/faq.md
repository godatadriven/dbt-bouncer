...
Yes! As `dbt-bouncer` runs on the artifacts generated by dbt, it is compatible with [dbt Fusion](https://docs.getdbt.com/docs/local/install-dbt), the Rust-based dbt engine. As long as dbt Fusion generates the standard dbt artifact files (`manifest.json`, `catalog.json`, `run_results.json`), `dbt-bouncer` will work without any changes to your configuration.
