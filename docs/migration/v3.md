...
> **Removed in v4:** the undocumented `from dbt_bouncer.cli import list_checks` lazy re-export is gone. Import it from its own module instead: `from dbt_bouncer.cli.list import list_checks`.
...
>
> **Removed in v4:** the undocumented `from dbt_bouncer.main import run_bouncer` alias is gone. Use `from dbt_bouncer import run_bouncer` (above) or the canonical `from dbt_bouncer.cli.run.utils import run_bouncer`.
...
| — | `dbt_bouncer.check_framework.decorator` (new) |
...
The decorator API is the recommended way to write custom checks in v3. Class-based checks (subclassing `BaseCheck`) still work as a legacy escape hatch for cases that need complex Pydantic validation, but they are not the encouraged path — see [class-based gotchas](#class-based-checks-legacy) below.
...
The `check_passes` / `check_fails` helpers take the check **name as a string** (its config name), followed by resource overrides and parameters as keyword arguments:
...
If your custom check lives in a `custom_checks_dir` rather than an installed plugin package, pass that directory so the helpers can find it — otherwise the check name will not resolve and you would have to monkeypatch the registry:

```python
def test_my_custom_check_pass():
    check_passes(
        "check_my_custom_check",
        custom_checks_dir="path/to/custom_checks",
        model={"name": "my_model"},
    )
```

### Class-based checks (legacy)

> **Removed in v4:** class-based checks (subclassing `BaseCheck` directly) now raise a `RuntimeError` at check discovery, naming the offending class and the module or file it came from, instead of being silently skipped. Migrate to the decorator API above — see [Adding a new check](../CONTRIBUTING.md#adding-a-new-check).

Class-based checks remain supported in v3 but are a legacy escape hatch — prefer the decorator API above. If you do subclass `BaseCheck`, two things commonly trip people up:

- **Re-declare the resource field on your subclass.** The runner infers which resource a class-based check iterates over from the subclass's *own* annotations (`__annotations__`), which Python does **not** inherit from `BaseCheck`. Calling `self._require_model()` is not enough — you must also declare the field, e.g. `model: Any = Field(default=None)`. Without it the check never iterates over any resource.
- **Use `Any` for the resource field when testing with the helpers.** The test helpers build resources as lightweight dicts; declaring the field as the strict artifact type (e.g. `ModelNode`) makes Pydantic reject those dicts during instantiation. Declaring `model: Any` (as the `@check` decorator does internally) avoids this.
