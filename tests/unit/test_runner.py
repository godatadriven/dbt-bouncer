...
def test_runner_check_with_error(caplog):
    ...
    assert results[0] == 0
    assert (
        "`dbt-bouncer` encountered an error (AttributeError: 'NoneType' object has no attribute 'find'), run with `-v` to see more details or report an issue at https://github.com/godatadriven/dbt-bouncer/issues."
        in caplog.text
    )
